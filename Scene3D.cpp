...
bool CScene3D::Update(const double dElapsedTime)
{
	...
	if (playerMoved == false)
	{
		...
		else if (CKeyboardController::GetInstance()->IsKeyPressed(GLFW_KEY_D))
		{
			playerMoved = true;
			cameraRight = true;
		}
	...
	if (playerMoved == true)
	{
		...
		else if (left && !CKeyboardController::GetInstance()->IsKeyPressed(GLFW_KEY_Q))
		{
			playerMoved = false;
			left = false;
		}
		else if (right && !CKeyboardController::GetInstance()->IsKeyPressed(GLFW_KEY_E))
		{
			playerMoved = false;
			right = false;
		}
		else if (cameraLeft && !CKeyboardController::GetInstance()->IsKeyPressed(GLFW_KEY_A))
		{
			playerMoved = false;
			cameraLeft = false;
			cPlayer3D->ProcessMovement(CPlayer3D::PLAYERMOVEMENT::CAMERALEFT, (float)dElapsedTime);
		}
		else if (cameraRight && !CKeyboardController::GetInstance()->IsKeyPressed(GLFW_KEY_D))
		{
			playerMoved = false;
			cameraRight = false;
...
void CScene3D::Render(void)
{
	...
	cProjectileManager->SetProjection(projection);		cProjectileManager->PreRender();
