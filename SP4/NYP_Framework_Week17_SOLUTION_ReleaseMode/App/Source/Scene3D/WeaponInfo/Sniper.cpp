...
bool CSniper::Init(void)
{
	...
	if (!success)
	{
		cout << "Unable to load Models/Sniper/sniperFPS.obj" << endl;
		return false;
	}
	...
	iconTextureID = CImageLoader::GetInstance()->LoadTextureGetID("Image/Icons/sniper.tga", false);
	if (iTextureID == 0)
	{
		cout << "Unable to load sniper icon" << endl;
		return false;
	}
	return true;
}

bool CSniper::Discharge(glm::vec3 vec3Position, glm::vec3 vec3Front, CSolidObject* pSource) {
	if (bFire)
	{
