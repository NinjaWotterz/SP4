...
CTree3D::CTree3D(const glm::vec3 vec3Position,
...
bool CTree3D::Init(void)
{
	...
	iTextureID = CImageLoader::GetInstance()->LoadTextureGetID("Models/Tree/tree.png", false);
	if (iTextureID == 0)
	{
		cout << "Unable to load Models/Tree/tree.png" << endl;
		return false;
	}
	...
	if (bInstancedRendering)
	{
		...
		for (unsigned int i = 0; i < iNumOfInstance; i++)
		{
			...
			// 3. Randomly rotate the Tree around a (semi)randomly picked rotation axis vector
 ...
 */
void CTree3D::SetModel(const glm::mat4 model)
{
	this->model = model;
}
 ...
 */
void CTree3D::SetView(const glm::mat4 view)
{
	this->view = view;
}
 ...
 */
void CTree3D::SetProjection(const glm::mat4 projection)
{
...
*/
void CTree3D::SetInstancingMode(const bool bInstancedRendering)
{
 ...
 */
void CTree3D::SetNumOfInstance(const unsigned int iNumOfInstance)
{
 ...
 */
void CTree3D::SetSpreadDistance(const float fSpreadDistance)
{
 ...
 */
bool CTree3D::Update(const double dElapsedTime)
{
	if (!bInstancedRendering)
	{
...
void CTree3D::Render(void)
{
	if (bInstancedRendering)
	{
		...
		// We instanced render the Tree. if you have multiple VAOs, then you can render them using this loop
		for (unsigned int i = 0; i < 1; i++)
		{
			glBindVertexArray(VAO);
...
/**
 @brief Print Self
 */
void CTree3D::PrintSelf(void)
{
	cout << "CTree3D::PrintSelf()" << endl;
	cout << "========================" << endl;
}
