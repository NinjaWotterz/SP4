...
class CFlyingObstacle : public CSolidObject, public CFSM
{
public:
	...
	enum class PATROLMOVEMENT : unsigned int
	{
		PATROLX = 0,
		PATROLZ,
		PATROLXZ,
	...
	// Default Constructor
	CFlyingObstacle(void);

	// Constructor with vectors
	CFlyingObstacle(const glm::vec3 vec3Position,
		const glm::vec3 vec3Front = glm::vec3(0.0f, 0.0f, -1.0f),
		const float fYaw = -90.0f,
		const float fPitch = 0.0f);

	// Destructor
	virtual ~CFlyingObstacle(void);
	...
	void SetPatrolMovement(PATROLMOVEMENT pm);
protected:
	// Enemy Attributes
	glm::vec3 vec3Up;
	glm::vec3 vec3Right;
	...
	PATROLMOVEMENT patrolMv; // to determine the waypoints pattern to patrol
