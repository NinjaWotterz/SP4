 ...
 */
CStatue3D::CStatue3D(	const glm::vec3 vec3Position,
 ...
 */
bool CStatue3D::Init(void)
{
	// Call the parent's Init()
	CSolidObject::Init();

	// Set the type
	SetType(CEntity3D::TYPE::STATUE);
	...
	std::string file_path = "Models/Enemy/statue.obj";
	bool success = CLoadOBJ::LoadOBJ(file_path.c_str(), vertices, uvs, normals, true);
	if (!success)
	{
		cout << "Unable to load Models/Enemy/statue.obj" << endl;
		return false;
	}
	...
	iTextureID = CImageLoader::GetInstance()->LoadTextureGetID("Image/statue.tga", false);
	if (iTextureID == 0)
	{
 ...
 */
void CStatue3D::SetModel(const glm::mat4 model)
{
	this->model = model;
}
 ...
 */
void CStatue3D::SetView(const glm::mat4 view)
{
	this->view = view;
}
 ...
 */
void CStatue3D::SetProjection(const glm::mat4 projection)
{
 ...
 */
void CStatue3D::AttachCamera(CCamera* cCamera)
{
 ...
 */
bool CStatue3D::IsCameraAttached(void)
{
	if (cCamera)
		return true;
	return false;
 ...
 */
void CStatue3D::SetWeapon(const int iSlot, CWeaponInfo* cWeaponInfo)
{
	if (iSlot == 0)
 ...
 */
CWeaponInfo* CStatue3D::GetWeapon(void) const
{
	if (iCurrentWeapon == 0)
...
/**
 ...
 @param iSlot A const int variable which contains the weapon info to check for. 0 == Primary, 1 == Secondary
 */
void CStatue3D::SetCurrentWeapon(const int iSlot)
{
...
/**
 ...
 @return A bool value
 */
bool CStatue3D::DischargeWeapon(void) const
{
 ...
 */
void CStatue3D::ProcessMovement(const ENEMYMOVEMENT direction, const float deltaTime)
{
 ...
 */
void CStatue3D::ProcessRotate(const float fXOffset)
{
	// Update the yaw
 ...
 */
bool CStatue3D::Update(const double dElapsedTime)
{
	...
	case FSM::IDLE:
		...
		angle = vec3Front.x + vec3Front.y + vec3Front.z + cPlayer3D->GetFront().x + cPlayer3D->GetFront().y + cPlayer3D->GetFront().z;

		if (glm::distance(vec3Position, cPlayer3D->GetPosition()) < fDetectionDistance && (angle <= -1 || angle >=1))
		{
 ...
 */
void CStatue3D::PostRender(void)
{
	// If this entity is not active, then skip this
	if (bStatus == false)
	{
 ...
 */
void CStatue3D::UpdateEnemyVectors(void)
{
 ...
 */
void CStatue3D::Constraint(void)
{
	// Get the new height
 ...
 */
void CStatue3D::UpdateFrontAndYaw(void)
{
